...
export function CalendarPicker({ selected, onSelect, allowFuture = false }: CalendarPickerProps) {
  ...
  function selectDay(day: number) {
    if (!allowFuture && isFuture(day)) return;
          ...
          {row.map((day, ci) =>
            day ? (
              <TouchableOpacity
                key={ci}
                activeOpacity={0.7}
                disabled={!allowFuture && isFuture(day)}
